...
앞 문단에서는 `Task`와 `async / await`를 구분하지 않고 사용했지만 사실은 차이가 있다. `async / await`는 언어 차원에서 제공되는 비동기 프로그래밍 기능에 관한 키워드이고, `Task`는 `async / await`를 이용해서 작업을 스레드풀에서 비동기적으로 실행할 수 있도록 해 주는 구현체이다. 따라서 꼭 `Task`가 아니더라도 `Task` 같은 무언가를 직접 만들어서 `async / await`를 통해 비동기 프로그래밍을 할 수 있다. 이 문단에서는 비동기 작업을 `MyTask<T>`라는 클래스를 만들어보면서 `Task`의 동작 원리에 대해 살펴 볼 것이다.
