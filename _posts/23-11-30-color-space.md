...
간단히 값을 대입해서 예시를 살펴보면, $$A=10$$, $$B=20$$일 때 사람이 $$A$$, $$B$$의 소리 크기 차이와 $$B$$, $$C$$의 소리 크기 차이를 동일하게 느끼려면 $$C=30$$이 아닌 $$C=40$$이어야 한다. 이러한 현상은 소리 뿐만 아니라 사람이 느끼는 모든 것에 적용된다. 전 재산이 만원일 때 만원이 더 생기는 것과 전 재산이 1억일 때 만원이 더 생기는 것을 비교해 보면 둘 다 똑같이 만원이 증가했는데도 느껴지는 것에 차이가 있는데, 그 원리가 바로 베버의 법칙이다.
